...
int main(void)
{
    ...
    const char* start_msg = TEXT("PRESS ANY KEY!");
    const char* end_msg = TEXT("Game Over!");
    ...
    wsprintf(str, TEXT(" score: %3d점"), meet);    // 점수를 출력하기 위해 문자열로 변환
    TextOut(hdc, 550, 100, str, lstrlen(str));
    ...
    while (!kbhit())
    {
        ...
        wsprintf(str, TEXT("%s"), start_msg);
        TextOut(hdc, 200, 250, str, lstrlen(str));
    }
    _getch();

    SelectObject(hdc, CreatePen(PS_SOLID, 3, RGB(255, 255, 255)));
    SelectObject(hdc, CreateSolidBrush(RGB(255, 255, 255)));
    Rectangle(hdc, 100, 250, 400, 270);
    ...
    while (1)
    {
        ...
        if (meet == 1)
        {
            wsprintf(str, TEXT(" score: %3d00점"), meet);    // 점수를 출력하기 위해 문자열로 변환
            TextOut(hdc, 550, 100, str, lstrlen(str));
        }
